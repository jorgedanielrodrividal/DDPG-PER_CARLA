...
For DDPG training, set `TRAIN_PLAY_MODE = 1` in `carla_config.py` and run

```
launch_DDPG.py
```

For DDPG evaluation, set `TRAIN_PLAY_MODE = 0` in `carla_config.py` and run

```
launch_DDPG.py
```

For DQN training, set `TRAIN_PLAY_MODE = 1` in `carla_config.py` and run

```
launch_DQN.py
```

For DQN evaluation, set `TRAIN_PLAY_MODE = 0` in `carla_config.py` and run
...
Therefore, at each step we add to optimal actions noise $x_t$ is given by $x_{t+1} = x_t + \theta(\mu - x_t) + \sigma \epsilon_t$, where $\theta, \mu, \sigma$ are hyperparameters and $\{\epsilon_t\}_t$ are i.i.d. random variables sampled from the normal distribution $\mathcal{N}(0, 1)$.
